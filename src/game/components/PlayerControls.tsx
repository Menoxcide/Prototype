/**
 * PlayerControls — Physics + Prediction + Real Ground
 * Handles player movement, jumping, running, crouching, wall-running, and climbing
 */
...
export default function PlayerControls() {
  ...
  const setIsCrouching = useGameStore((s) => s.setIsCrouching)
  const setIsWallRunning = useGameStore((s) => s.setIsWallRunning)
  ...
  const grapplePullVelocity = useGameStore((s) => s.grapplePullVelocity)
  const isOnCooldown = useGameStore((s) => s.isOnCooldown)
  const startCooldown = useGameStore((s) => s.startCooldown)
  const momentumOnRelease = useGameStore((s) => s.momentumOnRelease)
  const setMomentumOnRelease = useGameStore((s) => s.setMomentumOnRelease)
  const wallRunChain = useGameStore((s) => s.wallRunChain)
  const setWallRunChain = useGameStore((s) => s.setWallRunChain)
  const airDashCooldown = useGameStore((s) => s.airDashCooldown)
  const setAirDashCooldown = useGameStore((s) => s.setAirDashCooldown)
  const canAirDash = useGameStore((s) => s.canAirDash)
  ...
  const airDashDirection = useRef<THREE.Vector3 | null>(null)
  const isAirDashing = useRef<boolean>(false)
  const airDashDuration = useRef<number>(0)
  const AIR_DASH_COOLDOWN = 0.5 // 0.5 seconds cooldown
  const AIR_DASH_FORCE = 25 // Dash force
  const AIR_DASH_DURATION = 0.15 // Dash duration in seconds
  const DOUBLE_TAP_THRESHOLD = 0.3 // 300ms window for double-tap
  
  // Landing feedback
  const landingVelocity = useRef<number>(0)
  const collidableObjectsCache = useRef<THREE.Object3D[]>([])
  const lastCachePosition = useRef<{ x: number; y: number; z: number } | null>(null)
  const cacheFrameCount = useRef<number>(0)
  const wasGrappling = useRef<boolean>(false)  // Track previous grappling state
  const wasClimbing = useRef<boolean>(false)  // Track previous climbing state
  const wasRunning = useRef<boolean>(false)  // Track previous running state
  const wasCrouching = useRef<boolean>(false)  // Track previous crouching state
  const wasWallRunning = useRef<boolean>(false)  // Track previous wall-running state
  ...
  const shouldReengagePointerLock = useRef<boolean>(false) // Track if we should re-engage pointer lock after Alt is released
  ...
  useEffect(() => {
    ...
    const handleMouseDown = () => {
      // Don't request pointer lock if Alt is held (user wants to interact with UI)
      if (isAltPressed) {
        return
      }
        ...
        try {
          ...
          if (promise && promise instanceof Promise) {
            promise.catch((error: unknown) => {
              ...
              if (import.meta.env.DEV && error instanceof Error && error.name !== 'NotAllowedError') {
  ...
  }, [camera, cameraMode, isAltPressed])

  // Handle Alt key: release pointer lock when Alt is held, re-engage when released
  useEffect(() => {
    if (isMobile()) return

    if (isAltPressed) {
      // Alt is pressed - release pointer lock if active
      if (document.pointerLockElement !== null) {
        shouldReengagePointerLock.current = true
        document.exitPointerLock()
      }
    } else {
      // Alt is released - re-engage pointer lock if we were previously locked
      if (shouldReengagePointerLock.current && document.pointerLockElement === null) {
        shouldReengagePointerLock.current = false
        // Small delay to ensure UI interactions complete
        setTimeout(() => {
          try {
            const promise = document.body.requestPointerLock?.()
            if (promise && promise instanceof Promise) {
              promise.catch((error: unknown) => {
                if (import.meta.env.DEV && error instanceof Error && error.name !== 'NotAllowedError') {
                  console.debug('Pointer lock re-engagement failed:', error)
                }
              })
            }
          } catch (error) {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Movement keys - record input for lag profiling
      if (['w', 'a', 's', 'd'].includes(key)) {
        inputLagProfiler.recordInput(key)
        keysPressed.current.add(key)
      }
      
      // Jump (Space) - handle separately
      if (key === ' ' || key === 'space') {
        inputLagProfiler.recordInput('space')
      ...
      if (e.shiftKey) {
        ...
        if (!wasRunning.current) {
          import('../utils/combatLog').then(({ addStatusLog }) => {
            addStatusLog('Started running', '#ffaa00')
          })
      ...
      if ((key === 'e' || key === 'E') && !isOnCooldown('npc_interact')) {
        if (!isUIFocused()) {
          // Find nearest NPC
          if (player) {
            ...
            for (const npc of npcs.values()) {
              ...
              if (distance < 3 && (!nearestNPC || distance < nearestNPC.distance)) {
                nearestNPC = { id: npc.id, distance }
              }
            }
            
            if (nearestNPC) {
              const npcId = nearestNPC.id
              startNPCInteraction(npcId)
              startCooldown('npc_interact', COOLDOWN_NPC_INTERACT * 1000)
              e.preventDefault()
              e.stopPropagation()
            }
          }
      ...
      if (e.ctrlKey) {
        ...
        if (!wasCrouching.current) {
          import('../utils/combatLog').then(({ addStatusLog }) => {
            addStatusLog('Started crouching', '#888888')
          })
    ...
    const handleKeyUp = (e: KeyboardEvent) => {
      ...
      if (!e.shiftKey) {
        ...
        if (wasRunning.current) {
          import('../utils/combatLog').then(({ addStatusLog }) => {
            addStatusLog('Stopped running', '#888888')
          })
      ...
      if (!e.ctrlKey) {
        ...
        if (wasCrouching.current) {
          import('../utils/combatLog').then(({ addStatusLog }) => {
            addStatusLog('Stopped crouching', '#888888')
          })
  ...
  useFrame((_, delta) => {
    ...
    // Frame time smoothing - optimized to 7 frames for better responsiveness (reduced from 10)
    // This reduces input lag while still smoothing out jitter
    frameTimeHistory.current.push(delta)
    if (frameTimeHistory.current.length > 7) {
    ...
    // Record movement for input lag profiling
    if (moving) {
      for (const key of ['w', 'a', 's', 'd']) {
        if (keys.has(key)) {
          inputLagProfiler.recordMovement(key)
        }
      }
    }
    ...
    if (isRunning.current && !canRun) {
      ...
      // Log running stop due to stamina
      if (wasRunning.current) {
        import('../utils/combatLog').then(({ addStatusLog }) => {
          addStatusLog('Stopped running (out of stamina)', '#ff6666')
        })
        wasRunning.current = false
      }
    }
    ...
    // Can be disabled via feature flag to test if collision detection is causing stuttering
    const collisionDetectionEnabled = isFeatureEnabled('collisionDetectionEnabled')
    
    // Adaptive cache interval based on FPS - reduce frequency when FPS is low
    const fps = useGameStore.getState().fps
    const cacheInterval = fps > 0 && fps < 30 ? 30 : (fps > 0 && fps < 45 ? 20 : 10) // Increase interval when FPS is low
    
    // Cache collidable objects - adaptive frequency based on performance
    // Use spatial filtering to only cache objects near player
    cacheFrameCount.current++
    const shouldRecache = cacheFrameCount.current % cacheInterval === 0 || collidableObjectsCache.current.length === 0
    const playerMovedSignificantly = !lastCachePosition.current || (() => {
      const lastCachePos = lastCachePosition.current!
      const dx = Math.abs(player.position.x - lastCachePos.x)
      const dz = Math.abs(player.position.z - lastCachePos.z)
      return dx > 5 || dz > 5 // Only recache if player moved more than 5 units
    })()
    
    if (collisionDetectionEnabled && shouldRecache && playerMovedSignificantly) {
      ...
      // Limit traversal time to prevent blocking (max 5ms per frame)
      const traversalStartTime = performance.now()
      const MAX_TRAVERSAL_TIME = 5 // milliseconds
      
      scene.traverse((obj) => {
        // Early exit if traversal is taking too long
        if (performance.now() - traversalStartTime > MAX_TRAVERSAL_TIME) {
          return // Stop traversal to prevent frame drops
        }
      ...
      // Store last cache position to detect significant movement
      lastCachePosition.current = {
        x: player.position.x,
        y: player.position.y,
        z: player.position.z
      }
    ...
    const collidableObjects = collisionDetectionEnabled ? collidableObjectsCache.current : []
    ...
    const checkGround = (pos: THREE.Vector3): { height: number; hit: boolean } => {
      // If collision detection is disabled, return a default ground height
      if (!collisionDetectionEnabled) {
        return { height: 0, hit: true }
      }
      // Use cached result if position hasn't changed much and we checked recently
      // Adaptive caching: cache longer when FPS is low to reduce raycast frequency
      const cacheFrames = fps > 0 && fps < 30 ? 5 : 3 // Cache for more frames when FPS is low
      if (groundCheckCache.current) {
        ...
        // Cache valid for more frames if position changed less than 0.3 units
        if (posDiff < 0.3 && frameDiff < cacheFrames) {
    ...
    const canJump = !isOnCooldown('jump') && !isCrouching.current
    ...
    if (jumpPressed && canJump && canJumpWithStamina && (onGround || isWallRunning.current)) {
      ...
      isGrounded.current = false
      startCooldown('jump', COOLDOWN_JUMP * 1000)
      setIsJumping(true)
      ...
      playMovementSound('jump', 0.5)
      
      // Log jump status
      import('../utils/combatLog').then(({ addStatusLog }) => {
        addStatusLog('Jumped', '#00ff88')
      })
    }
    
    // Update jump state - clear when landing (transitioning from air to ground)
    const wasInAir = !isGrounded.current
    if (onGround && wasInAir) {
      setIsJumping(false)
      
      // Landing feedback: screen shake + visual effect based on landing velocity
      if (landingVelocity.current > 5) {
        const shakeIntensity = Math.min(0.15, landingVelocity.current / 50)
        setScreenShakeIntensity(shakeIntensity)
        
        // Haptic feedback for landing (intensity based on velocity)
        const hapticIntensity = Math.min(2, landingVelocity.current / 10)
        hapticManager.landing(hapticIntensity)
        
        // Sound effect for landing (volume based on velocity)
        const landingVolume = Math.min(1.0, landingVelocity.current / 20)
        playMovementSound('landing', landingVolume)
        
        // Trigger landing feedback component
        import('../components/LandingFeedback').then(({ triggerLanding }) => {
          triggerLanding(player.position, landingVelocity.current)
        }).catch(() => {
          // Component not created yet, skip
        })
      }
      
      // Reset wall-run chain on ground contact
      setWallRunChain(0)
    }
    
    // Track landing velocity
    if (!isGrounded.current) {
      landingVelocity.current = Math.abs(velocity.current.y)
    } else {
      landingVelocity.current = 0
    }

    // === AIR DASH SYSTEM ===
    // Update air dash cooldown
    if (airDashCooldown > 0) {
      setAirDashCooldown(Math.max(0, airDashCooldown - smoothedDelta.current))
    }
    
    // Update air dash state
    if (isAirDashing.current) {
      airDashDuration.current += smoothedDelta.current
      if (airDashDuration.current >= AIR_DASH_DURATION) {
        isAirDashing.current = false
        airDashDuration.current = 0
        airDashDirection.current = null
      } else if (airDashDirection.current) {
        // Apply dash velocity
        const dashSpeed = AIR_DASH_FORCE * (1 - airDashDuration.current / AIR_DASH_DURATION) // Fade out
        velocity.current.x += airDashDirection.current.x * dashSpeed * smoothedDelta.current
        velocity.current.y += airDashDirection.current.y * dashSpeed * smoothedDelta.current
        velocity.current.z += airDashDirection.current.z * dashSpeed * smoothedDelta.current
      }
    }
    
    // Double-tap detection for air dash
    if (!isGrounded.current && !isAirDashing.current && airDashCooldown <= 0 && canAirDash && stamina >= 30) {
      const dashKeys = ['w', 'a', 's', 'd', 'ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight']
      for (const key of dashKeys) {
        if (keys.has(key)) {
          const now = Date.now()
          const lastPress = lastKeyPressTime.current.get(key) || 0
          
          if (now - lastPress < DOUBLE_TAP_THRESHOLD * 1000) {
            // Double-tap detected - perform air dash
            const dashDir = getVector3()
            if (key === 'w' || key === 'ArrowUp') dashDir.z -= 1
            if (key === 's' || key === 'ArrowDown') dashDir.z += 1
            if (key === 'a' || key === 'ArrowLeft') dashDir.x -= 1
            if (key === 'd' || key === 'ArrowRight') dashDir.x += 1
            ...
            if (dashDir.length() > 0) {
              ...
              const cameraEuler = getEuler().setFromQuaternion(camera.quaternion, 'YXZ')
              const yawEuler = getEuler(0, cameraEuler.y, 0)
              dashDir.applyEuler(yawEuler)
              releaseEuler(cameraEuler)
              releaseEuler(yawEuler)
            
              airDashDirection.current = dashDir.clone()
              isAirDashing.current = true
              airDashDuration.current = 0
              setAirDashCooldown(AIR_DASH_COOLDOWN)
              consumeStamina(30)
              
              // Haptic feedback for air dash
              hapticManager.airDash()
              
              // Sound effect for air dash
              playMovementSound('airDash', 0.7)
              
              import('../utils/combatLog').then(({ addStatusLog }) => {
                addStatusLog('Air Dash!', '#00ffff')
              })
            }
            releaseVector3(dashDir)
            break
          }
          
          lastKeyPressTime.current.set(key, now)
        }
      }
    }
    
    // === GRAPPLE PULL FORCE ===
    // Apply grapple pull acceleration if grappling (adds to existing velocity)
    // The pull is an acceleration (force), so multiply by delta to get velocity change
    if (grapplePullVelocity) {
      velocity.current.x += grapplePullVelocity.x * smoothedDelta.current
      velocity.current.y += grapplePullVelocity.y * smoothedDelta.current
      velocity.current.z += grapplePullVelocity.z * smoothedDelta.current
    }
    
    // === MOMENTUM CONSERVATION ===
    // Apply stored momentum when grapple is released
    if (momentumOnRelease) {
      velocity.current.x += momentumOnRelease.x
      velocity.current.y += momentumOnRelease.y
      velocity.current.z += momentumOnRelease.z
      // Clear momentum after applying (one-time boost)
      setMomentumOnRelease(null)
    }
    ...
    // When grappling, still apply gravity (grapple pull will counteract it)
    if (!isGrounded.current && !isClimbingBuilding.current) {
      ...
      velocity.current.y = 0
      // Clear jump state when grounded
      setIsJumping(false)
    }
    ...
    // === WALL-RUNNING ===
    const wasWallRunningBefore = isWallRunning.current
    isWallRunning.current = false
    if (wallCheck.hit && wallCheck.normal && !isGrounded.current && moveDirection.length() > 0.1) {
      ...
      if (wallAngle < 0.7) { // Wall is more vertical than horizontal
        ...
        if (dotProduct < -0.3) { // Moving towards wall
          ...
          // Wall-run chain tracking
          if (!wasWallRunning.current) {
            // Starting a new wall-run - increment chain
            setWallRunChain(wallRunChain + 1)
            // Haptic feedback for wall-run
            hapticManager.wallRun()
            // Sound effect for wall-run (continuous)
            playMovementSound('wallRun', 0.6)
          }
          
          // Log wall-running start
          if (!wasWallRunning.current) {
            import('../utils/combatLog').then(({ addStatusLog }) => {
              addStatusLog('Started wall-running', '#ff00ff')
            })
            wasWallRunning.current = true
          }
    ...
    // Update wall-running state - clear when not wall-running
    if (!isWallRunning.current && wasWallRunningBefore) {
      setIsWallRunning(false)
      
      // Log wall-running stop
      if (wasWallRunning.current) {
        import('../utils/combatLog').then(({ addStatusLog }) => {
          addStatusLog('Stopped wall-running', '#888888')
        })
        wasWallRunning.current = false
      }
    }
    ...
    if (wallCheck.hit && wallCheck.isBuilding && wallCheck.normal && moveDirection.length() > 0.1) {
      ...
      if (wallAngle < 0.7) { // Wall is more vertical than horizontal
        ...
        if (dotProduct < -0.3) { // Moving towards building
          ...
          // Log climbing start
          if (!wasClimbing.current) {
            import('../utils/combatLog').then(({ addStatusLog }) => {
              addStatusLog('Started climbing', '#ff8800')
            })
          }
          ...
          if (keys.has('w') || keys.has('ArrowUp')) {
            ...
            if (wasClimbing.current) {
              import('../utils/combatLog').then(({ addStatusLog }) => {
                addStatusLog('Stopped climbing', '#888888')
              })
            }
    ...
    if (moving || posDelta > 0.001) {
      ...
      // Update prefetching system with player position and velocity
      progressiveLoader.updatePlayerPosition(clampedPos, {
