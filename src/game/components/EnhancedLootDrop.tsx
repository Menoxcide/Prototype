...
interface EnhancedLootDropProps {
  loot: LootDropType
}

export default function EnhancedLootDrop({ loot }: EnhancedLootDropProps) {
  ...
  useFrame((state, delta) => {
    if (meshRef.current) {
      meshRef.current.position.set(
        loot.position.x,
        loot.position.y + Math.sin(state.clock.elapsedTime * 2) * 0.3,
        loot.position.z
      )
  ...
  return (
    <group ref={meshRef}>
      {/* Enhanced loot item mesh with better geometry */}
      <mesh>
